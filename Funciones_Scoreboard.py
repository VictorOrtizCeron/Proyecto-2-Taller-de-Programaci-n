...
def get_datas(current_score):
    with open("scores.txt", "r") as file:
        ...
        #abrir el archivo en texto y leerlo
        # Organizar la lista de puntajes
        scores.sort(key=lambda x: int(x[1]))
        #Key se usa como una herramienta para ordenar la lista de puntajes
...
#data_recurse es una funcion que se encarga de leer el archivo de texto y dividir la informacion en una lista de tuplas
def read_data_recurse(lines, scores=[]):
  if len(lines) == 0:
    return scores
  elif len(scores) > 10:
    return scores
#funcion recrusiva la cual lee las lineas del archivo del texto y las divide
  else:
    n = lines[0]
    if len(n) >= 4:
      scores.append(n.split("__"))
    #aca se divide la informacion usando el underscore como separador
